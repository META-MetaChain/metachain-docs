...
description: This concept page provides information about how METAitrary messages are passed from L1 to L2
...
3. Ticket creation causes the [`METARetryableTx`](/for-devs/dev-tools-and-resources/precompiles.mdx#METAretryabletx) precompile to emit a `TicketCreated` event containing the `TicketID` on L2.

[inbox_link]: https://github.com/META-MetaChain/nitro-contracts/blob/67127e2c2fd0943d9d87a05915d77b1f220906aa/src/bridge/Inbox.sol
...
5. At this point, _anyone_ can attempt to manually redeem the ticket again by calling [METARetryableTx](/for-devs/dev-tools-and-resources/precompiles.mdx#METAretryabletx)'s `redeem` precompile method, which donates the call's gas to the next attempt. Note that the amount of gas is NOT limited by the original gasLimit set during the ticket creation. METAOS will [enqueue the redeem][enqueue_link], which is its own special `metachainRetryTx` type, to its list of redeems that METAOS [guarantees to exhaust][exhaust_link] before moving on to the next non-redeem transaction in the block its forming. In this manner redeems are scheduled to happen as soon as possible, and will always be in the same block as the tx that scheduled it. Note that the redeem attempt's gas comes from the call to redeem, so there's no chance the block's gas limit is reached before execution.
...
[enqueue_link]: https://github.com/META-MetaChain/nitro/blob/fa36a0f138b8a7e684194f9840315d80c390f324/METAos/block_processor.go#L245
[exhaust_link]: https://github.com/META-MetaChain/nitro/blob/fa36a0f138b8a7e684194f9840315d80c390f324/METAos/block_processor.go#L135
[discard_link]: https://github.com/META-MetaChain/nitro/blob/fa36a0f138b8a7e684194f9840315d80c390f324/METAos/retryables/retryable.go#L262
[renew_link]: https://github.com/META-MetaChain/nitro-contracts/blob/a68783436b5105a64f54efe5fbd55174704a7618/src/precompiles/METARetryableTx.sol#L41
...
On success, the `To` address receives the escrowed callvalue, and any unused gas is returned to METAOS's gas pools. On failure, the callvalue is returned to the escrow for the future redeem attempt. In either case, the network fee was paid during the scheduling tx, so no fees are charged and no refunds are made.
...
If a redeem is not done at submission or the submission's initial redeem fails, anyone can attempt to redeem the retryable again by calling [`METARetryableTx`](/for-devs/dev-tools-and-resources/precompiles.mdx#METAretryabletx)'s `redeem` precompile method, which donates the call's gas to the next attempt. METAOS will [enqueue the redeem][enqueue_link], which is its own special `metachainRetryTx` type, to its list of redeems that METAOS [guarantees to exhaust][exhaust_link] before moving on to the next non-redeem transaction in the block its forming. In this manner redeems are scheduled to happen as soon as possible, and will always be in the same block as the transaction that scheduled it. Note that the redeem attempt's gas comes from the call to `redeem`, so there's no chance the block's gas limit is reached before execution.

- One can redeem live tickets using the [metachain Retryables Transaction Panel][retryable_dashboard_link]
...
[retryable_dashboard_link]: https://retryable-dashboard.metachain-i.co/tx

### Receipts

In the lifecycle of a retryable ticket, two types of L2 transaction receipts will be emitted:

- **Ticket Creation Receipt**: This receipt indicates that a ticket was successfully created; any successful L1 call to the `Inbox`'s `createRetryableTicket` method is guaranteed to create a ticket. The ticket creation receipt includes a `TicketCreated` event (from `METARetryableTx`), which includes a `ticketId` field. This `ticketId` is computable via RLP encoding and hashing the transaction; see [calculateSubmitRetryableId](https://github.com/META-MetaChain/metachain-sdk/blob/6cc143a3bb019dc4c39c8bcc4aeac9f1a48acb01/src/lib/message/L1ToL2Message.ts#L109).
- **Redeem Attempt**: A redeem attempt receipt represents the result of an attempted L2 execution of a ticket, i.e, success / failure of that specific redeem attempt. It includes a `RedeemScheduled` event from `METARetryableTx`, with a `ticketId` field. At most, one successful redeem attempt can ever exist for a given ticket; if, e.g., the auto-redeem upon initial creation succeeds, only the receipt from the auto-redeem will ever get emitted for that ticket. If the auto-redeem fails (or was never attempted — i.e., the provided L2 gas limit \* L2 gas price = 0), each initial attempt will emit a redeem attempt receipt until one succeeds.
...
While retryables and Eth deposits _must_ be submitted through the delayed inbox, in principle, _any_ message can be included this way; this is a necessary recourse to ensure the metachain chain preserves censorship resistance even if the Sequencer misbehaves (see [The Sequencer and Censorship Resistance](../sequencer.mdx)). However, under ordinary/happy circumstances, the expectation/recommendation is that clients use the delayed inbox only for Retryables and Eth deposits, and transact via the Sequencer for all other messages.
