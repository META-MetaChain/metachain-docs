...
description: Learn how to set up a custom gateway using metachain's Token Bridge to bridge tokens programmatically
reader_audience: developers who want to build on Ethereum/metachain and bridge tokens between layers
content_type: how-to
---
...
Before starting to implement and deploy a custom gateway, it is strongly encouraged to analyze the current solutions that metachain’s token bridge provides: the [standard gateway](/devs-how-tos/bridge-tokens/how-to-bridge-tokens-standard.mdx) and the [generic-custom gateway](/devs-how-tos/bridge-tokens/how-to-bridge-tokens-generic-custom.mdx). These solutions provide enough functionality to solve the majority of bridging needs from projects. And if you are in doubt about your current approach, you can always ask for assistance on our [Discord server](https://discord.gg/metachain).

:::

In this how-to you’ll learn how to bridge your own token between Ethereum (Layer 1 or L1) and metachain (Layer 2 or L2), using a custom gateway. For alternative ways of bridging tokens, don’t forget to check out this [overview](/devs-how-tos/bridge-tokens/how-to-bridge-tokens-overview.mdx).

Familiarity with [metachain’s token bridge system](/for-devs/concepts/token-bridge/token-bridge-overview.mdx), smart contracts, and blockchain development is expected. If you’re new to blockchain development, consider reviewing our [Quickstart: Build a dApp with metachain (Solidity, Hardhat)](/for-devs/quickstart-solidity-hardhat.md) before proceeding. We will use [metachain’s SDK](https://github.com/META-MetaChain/metachain-sdk) throughout this how-to, although no prior knowledge is required.

We will go through all steps involved in the process. However, if you want to jump straight to the code, we have created [this script in our tutorials repository](https://github.com/META-MetaChain/metachain-tutorials/tree/master/packages/custom-gateway-bridging) that encapsulates the entire process.
...
First of all, the **L1 counterpart of the gateway**, must conform to the [IL1metachainGateway](https://github.com/META-MetaChain/token-bridge-contracts/blob/main/contracts/tokenbridge/ethereum/gateway/IL1metachainGateway.sol) and the [ITokenGateway](https://github.com/META-MetaChain/token-bridge-contracts/blob/main/contracts/tokenbridge/libraries/gateway/ITokenGateway.sol) interfaces. This means that it must have, at least:
...
- Methods to send cross-chain messages through the [inbox contract](https://github.com/META-MetaChain/nitro-contracts/blob/main/src/bridge/Inbox.sol). An example implementation can be found in `sendTxToL2` and `sendTxToL2CustomRefund` on [L1metachainMessenger](https://github.com/META-MetaChain/token-bridge-contracts/blob/main/contracts/tokenbridge/ethereum/L1metachainMessenger.sol).

Furthermore, if you plan on having permissionless registration of tokens in your gateway, your L1 gateway should also have a `registerCustomL2Token` method, similar to the one being used in metachain’s [generic-custom gateway](https://github.com/META-MetaChain/token-bridge-contracts/blob/main/contracts/tokenbridge/ethereum/gateway/L1CustomGateway.sol#L146).

On the other hand, the **L2 counterpart of the gateway**, must conform to the [ITokenGateway](https://github.com/META-MetaChain/token-bridge-contracts/blob/main/contracts/tokenbridge/libraries/gateway/ITokenGateway.sol) interface. This means that it must have, at least:
...
- Two methods `calculateL2TokenAddress` and `getOutboundCalldata` to handle other bridging operations.
- Methods to send cross-chain messages through the [METASys precompile](/for-devs/dev-tools-and-resources/precompiles.mdx#METAsys). An example implementation can be found in `sendTxToL1` on [L2metachainMessenger](https://github.com/META-MetaChain/token-bridge-contracts/blob/main/contracts/tokenbridge/metachain/L2metachainMessenger.sol).

### What about my custom tokens?

If you are deploying custom gateways, you will probably want to support your custom tokens on L1 and L2 too. They also have several requirements they must comply with. You can find more information about it in [How to bridge tokens via metachain’s generic-custom gateway](/devs-how-tos/bridge-tokens/how-to-bridge-tokens-generic-custom.mdx).
...
const { getL2Network, L1ToL2MessageStatus } = require('@metachain/sdk');
const {
  AdminErc20Bridger,
  Erc20Bridger,
} = require('@metachain/sdk/dist/lib/assetBridger/erc20Bridger');
...
const main = async () => {
  /**
   * Use l2Network to create an metachain SDK AdminErc20Bridger instance
...
We’ll now create the counterpart of the gateway we created on L1 and deploy it on L2. A good example of a custom gateway on L2 is [metachain’s generic-custom gateway on L2](https://github.com/META-MetaChain/token-bridge-contracts/blob/main/contracts/tokenbridge/metachain/gateway/L2CustomGateway.sol).
...
import "./interfaces/ICustomGateway.sol";
import "./CrosschainMessenger.sol";
import "./interfaces/IMETAToken.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
...
contract L2CustomGateway is IL2CustomGateway, L2CrosschainMessenger, Ownable {
    ...
    function outboundTransfer(
        ...
        IMETAToken(l2CustomToken).bridgeBurn(from, amount);
    ...
    function finalizeInboundTransfer(
        ...
        // Mints L2 tokens
        IMETAToken(l2CustomToken).bridgeMint(to, amount);
...
`IL2CustomGateway` is also an interface very similar to `ICustomGateway`, and `L2CrosschainMessenger` implements a method to send the cross-chain message to L1 through METASys.
...
abstract contract L2CrosschainMessenger {
    address internal constant META_SYS_ADDRESS = address(100);
    ...
    /**
     * Creates an L2-to-L1 message to send over to L1 through METASys
    ...
    function _sendTxToL1(
        ...
        uint256 id = METASys(META_SYS_ADDRESS).sendTxToL1(to, data);
...
We won’t go through the process of deploying custom tokens in this How-to, but you can see a detailed explanation of the steps to take in the page [How to bridge tokens via metachain’s generic-custom gateway](/devs-how-tos/bridge-tokens/how-to-bridge-tokens-generic-custom.mdx)
...
As mentioned in [How to bridge tokens via metachain’s generic-custom gateway](/devs-how-tos/bridge-tokens/how-to-bridge-tokens-generic-custom.mdx), this action needs to be done by the L1 token, and we’ve implemented the function `registerTokenOnL2` to do it. So now we only need to call that function.
...
To simplify the process, we’ll use metachain’s SDK and call the method [registerCustomToken](/sdk/assetBridger_erc20Bridger#registercustomtoken) of the [AdminErc20Bridger](/sdk/assetBridger_erc20Bridger#adminerc20bridger) class, which will call the registerTokenOnL2 method of the token passed by parameter.
...
If you want to see an example of bridging a token from L1 to L2 using metachain’s SDK, you can check out [How to bridge tokens via metachain’s standard ERC20 gateway](/devs-how-tos/bridge-tokens/how-to-bridge-tokens-standard.mdx), where the process is described in steps 2-5.

The full code of this how-to and a more extensive deployment and testing script can be found [in this package of our tutorials repository](https://github.com/META-MetaChain/metachain-tutorials/tree/master/packages/custom-gateway-bridging).

## Resources

1. [Concept page: Token Bridge](/for-devs/concepts/token-bridge/token-bridge-overview.mdx)
2. [metachain SDK](https://github.com/META-MetaChain/metachain-sdk)
