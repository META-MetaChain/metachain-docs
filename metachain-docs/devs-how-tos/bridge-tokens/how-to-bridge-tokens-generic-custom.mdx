---
title: 'How to bridge tokens via metachain’s generic-custom gateway'
sidebar_label: Generic-custom gateway
...
In this how-to you’ll learn how to bridge your own token between Ethereum (Layer 1 or L1) and metachain (Layer 2 or L2), using [metachain’s generic-custom gateway](/for-devs/concepts/token-bridge/token-bridge-erc20.mdx#the-metachain-generic-custom-gateway). For alternative ways of bridging tokens, don’t forget to check out this [overview](/devs-how-tos/bridge-tokens/how-to-bridge-tokens-overview.mdx).
...
We will go through all steps involved in the process. However, if you want to jump straight to the code, we have created [this script in our tutorials repository](https://github.com/META-MetaChain/metachain-tutorials/tree/master/packages/custom-token-bridging) that encapsulates the entire process.

## Step 0: Review the prerequisites

As stated in the [token bridge conceptual page](/for-devs/concepts/token-bridge/token-bridge-erc20.mdx#the-metachain-generic-custom-gateway), there are a few prerequisites to keep in mind while using this method to make a token bridgeable.

First of all, the **L1 counterpart of the token**, must conform to the [ICustomToken](https://github.com/META-MetaChain/token-bridge-contracts/blob/main/contracts/tokenbridge/ethereum/ICustomToken.sol) interface. This means that:

- It must have a `ismetachainEnabled` method that returns `0xb1`
...
- As a chain-owner registration via an [metachain DAO](https://forum.metachain.foundation/) proposal.
...
Also, the **L2 counterpart of the token**, must conform to the [IMETAToken](https://github.com/META-MetaChain/token-bridge-contracts/blob/main/contracts/tokenbridge/metachain/IMETAToken.sol) interface. This means that:
...
contract L1Token is Ownable, ICustomToken, ERC20 {
    ...
    function ismetachainEnabled() external view override returns (uint8) {
...
const { getL2Network } = require('@metachain/sdk');
...
We’ll create a standard ERC20 contract using OpenZeppelin’s implementation, and add the required methods from `IMETAToken`.
...
contract L2Token is ERC20, IMETAToken {
...
To simplify the process, we’ll use metachain’s SDK. We’ll call the method [registerCustomToken](/sdk/assetBridger_erc20Bridger#registercustomtoken) of the [AdminErc20Bridger](/sdk/assetBridger_erc20Bridger#adminerc20bridger) class, which will call the registerTokenOnL2 method of the token passed by parameter.
...
As mentioned in the concept page, the token registration can alternatively be performed as a chain-owner registration via **[metachain DAO](https://forum.metachain.foundation/)** proposal.
