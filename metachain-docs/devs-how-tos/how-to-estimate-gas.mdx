...
title: 'How to estimate gas in metachain'
...
This how-to is intended for users and developers interested in understanding how gas operates in metachain, how it's calculated, and how to estimate it before submitting transactions. More detailed information about these calculations can be found in this [Medium article](https://medium.com/meta-research/understanding-metachain-2-dimensional-fees-fd1d582596c9) and the [Gas and Fees](/METAos/gas.mdx) page.

## Skip the formula, focus on practical know-how

Before diving into the specifics and the formula, if you're looking for a practical way to estimate gas for your transaction, you can rely on the standard gas estimation process. This can be achieved by calling an metachain node's `eth_estimateGas`, which provides a value (gas limit) that should sufficiently cover the entire transaction fee at the specified L2 gas price.
...
Note that when working with L1 to L2 messages (also known as [retryable tickets](/METAos/l1-to-l2-messaging.mdx)), you can use the function [L1ToL2MessageGasEstimator.estimateAll()](https://github.com/META-MetaChain/metachain-sdk/blob/main/src/lib/message/L1ToL2MessageGasEstimator.ts#L215) of the metachain SDK or [NodeInterface.estimateRetryableTicket()](https://github.com/META-MetaChain/@nitroRepositorySlug@/blob/@nitroVersionTag@/nodeInterface/NodeInterface.go#L120) to get all the gas information needed to send a successful transaction.
...
However, if you want to jump straight to the code, we have created [this script in our tutorials repository](https://github.com/META-MetaChain/metachain-tutorials/tree/master/packages/gas-estimation) that goes through all the calculations explained in this how-to.
...
This Gas Limit includes the gas of the L2 computation and an additional buffer to cover the L1 gas to be paid by the Sequencer when [posting the batch including this transaction on L1](/inside-metachain-nitro/inside-metachain-nitro.mdx#how-the-sequencer-publishes-the-sequence).
...
More information is available [in this page](/METAos/l1-pricing.mdx).
...
We'll use one resource available in metachain: the [NodeInterface](/for-devs/dev-tools-and-resources/nodeinterface.mdx).

- P (L2 Gas Price) ⇒ Price to pay for each gas unit. It starts at @METAOneGasFloorGwei@ gwei on metachain One (@novaGasFloorGwei@ gwei on metachain Nova) and can increase depending on the demand for network resources.
...
- L1S (Size of data to be posted on L1, in bytes) ⇒ This will depend on the data of the transaction. Keep in mind that metachain adds a fixed amount to this number to make up for the static part of the transaction, which is also posted on L1 (140 bytes). We can do a small calculation to obtain this value: call `NodeInterface.GasEstimateComponents()` take the first element, `gasEstimateForL1` (this is equivalent to `B` in our formula), multiply it by P and divide it by L1P.
  - For metachain Nova (AnyTrust), the size of the data is also a fixed value, as only the Data Availability Certificate is posted on L1, [as explained here](/inside-anytrust#data-availability-certificates).

## An example of how to apply this formula in your code

Finally, we show an example of how to get the values we just described and how to estimate the gas usage of a transaction in Javascript. We'll use our [SDK](https://github.com/META-MetaChain/metachain-sdk) to connect to the `NodeInterface`.
...
```ts
const { NodeInterface__factory } = require("@metachain/sdk/dist/lib/abi/factories/NodeInterface__factory");
const { NODE_INTERFACE_ADDRESS } = require("@metachain/sdk/dist/lib/dataEntities/constants");
...
```ts
// Getting the gas prices from METAGasInfo.getPricesInWei()
const gasComponents = await METAGasInfo.callStatic.getPricesInWei();
...
Refer to [our tutorials repository](https://github.com/META-MetaChain/metachain-tutorials/tree/master/packages/gas-estimation) for a working example of this code.
