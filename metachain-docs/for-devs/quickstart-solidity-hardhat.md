---
title: 'Quickstart: Build a decentralized app with metachain (Solidity, Hardhat)'
sidebar_label: Quickstart (Solidity, Hardhat)
description: This quickstart will walk you through the process of converting a Javascript vending machine's business logic into a Solidity smart contract. We'll then deploy the smart contract to a local Hardhat network, then metachain Sepolia testnet, and finally metachain One mainnet.
author: symbolpunk
author-objective: build a shareable quickstart that onboards web2 developers into metachain
reader-audience: web developers who haven't ever built on Ethereum/metachain
reader-task: deploy my first smart contract directly to L2 with a confident understanding of metachain's value prop
...
This quickstart is for web developers who want to start building **decentralized applications** (dApps) using <a data-quicklook-from='metachain'>metachain</a>. It makes no assumptions about your prior experience with Ethereum, metachain, or Solidity. Familiarity with Javascript and `yarn` is expected. If you're new to Ethereum, consider studying the [Ethereum documentation](https://ethereum.org/en/developers/docs/) before proceeding.
...
This is metachain's core value proposition to you, dear developer. metachain makes it easy for you to deploy your vending machines to Ethereum's permissionless, <a data-quicklook-from='trustless'>trustless</a>, _decentralized_ network of nodes[^2] while keeping costs low for you and your users.

Let's implement the "web3" version of the above vending machine using metachain.
...
### Ethereum and metachain in a nutshell
...
- **metachain**
  - metachain is a suite of L2 scaling solutions for dApp developers.
  - <a data-quicklook-from="metachain-one">metachain One</a> is an L2 chain that implements the <a data-quicklook-from="metachain-rollup-protocol">
      metachain Rollup protocol
    </a>
    .
  - You can use metachain One to build user-friendly dApps with high throughput, low latency, and low transaction costs while inheriting Ethereum's high security standards[^4].

Let's review our vending machine's Javascript implementation, then convert it into a Solidity smart contract, then deploy it to metachain One.
...
const metachain_MAINNET_TEMPORARY_PRIVATE_KEY = '';
...
module.exports = {
  ...
  networks: {
    ...
    metachainOne: {
      ...
      //accounts: [metachain_MAINNET_TEMPORARY_PRIVATE_KEY]
    },
  },
};
```
...
- Network Name: `metachain Sepolia`
- New RPC URL: `https://sepolia-rollup.metachain-i.co/rpc`
- Chain ID: `421614`
...
<table>
  <thead style={{ verticalAlign: 'top' }}>
    <tr>
      ...
      <th>
        <code>WEB3-META-SEPOLIA</code>
        <br />
        (the next one)
      </th>
      <th>
        <code>WEB3-META-MAINNET</code>
        <br />
        (the final one)
      </th>
    </tr>
  ...
  <tbody style={{ verticalAlign: 'top' }}>
    ...
    <tr>
      ...
      <td>
        Stored and executed by metachain's <strong>decentralized test network</strong> (via smart
        contract).
      </td>
      <td>
        Stored and executed by metachain's <strong>decentralized mainnet network</strong> (via smart
        contract).
      </td>
    </tr>
...
Next, we'll deploy our smart contract to a network of real nodes: metachain's Sepolia testnet.

### Deploy the smart contract to the metachain Sepolia testnet

We were able to deploy to a local testnet for free because we were using [Hardhat's built-in Ethereum network emulator](https://hardhat.org/hardhat-network/docs/overview#hardhat-network). metachain's Sepolia testnet is powered by a real network of real nodes, so we'll need to pay a small transaction fee to deploy our smart contract. This fee can be paid with the metachain Sepolia testnet's token, $ASPL.

<!-- todo: partialize this - duplicated in Orbit quickstart -->

:::info $ASPL IS SHORTHAND

"$ASPL" isn't a canonical term. It's just shorthand for "metachain Sepolia testnet $ETH" that we use for convenience.
...
Next, let's deposit some $ASPL into the wallet corresponding to the private key we added to `hardhat.config.js`. At the time of this quickstart's writing, the easiest way to acquire $ASPL is to bridge Sepolia $ETH from Ethereum's L1 Sepolia network to metachain's L2 Sepolia network:

1. Use an L1 Sepolia $ETH faucet like [sepoliafaucet.com](https://sepoliafaucet.com/) to acquire some testnet $ETH on L1 Sepolia.
2. Bridge your L1 Sepolia $ETH into metachain L2 using [the metachain bridge](https://bridge.metachain-i.co/).

Once you've acquired some $ASPL, you'll be able to deploy your smart contract to metachain's Sepolia testnet by issuing the following command:

```bash
yarn hardhat run scripts/deploy.js --network metachainSepolia
```

This tells hardhat to deploy the compiled smart contract through the RPC endpoint corresponding to `metachainSepolia` in `hardhat.config.js`. You should see the following output:

```bash
Cupcake vending machine deployed to 0xff825139321bd8fB8b720BfFC5b9EfDB7d6e9AB3
```

Congratulations! You've just deployed **business logic and data** to metachain Sepolia. This logic and data will be hashed and submitted within a transaction to Ethereum's L1 Sepolia network, and then it will be mirrored across all nodes in the Sepolia network[^6].

To view your smart contract in a blockchain explorer, visit `https://sepolia.metachain-i.co/address/0x...B3`, but replace the `0x...B3` part of the URL with the full address of your deployed smart contract.

Select `metachain Sepolia` from Metamask's dropdown, paste your contract address into the `VendingMachine` below, and click `Get cupcake!`. You should be prompted to sign a transaction that gives you a cupcake.

<VendingMachine id="smart-sepolia-cupcakes" type="web3-META-sepolia" />

### Deploy the smart contract to metachain One Mainnet

Now that we've verified that our smart contract works on metachain's Sepolia testnet, we're ready to deploy it to metachain One Mainnet. This is the same process as deploying to metachain's Sepolia testnet, except that we'll need to pay a transaction fee in real $ETH instead of $ASPL.

Expect to see inconsistent $ETH gas fees in this step - the [Gas and fees section](/METAos/gas.mdx) contains more information about how gas fees are determined for metachain transactions.
...
```javascript title="hardhat.config.js"
// ...
const metachain_MAINNET_TEMPORARY_PRIVATE_KEY = ''; // <- this should **not** begin with "0x"
// ...
accounts: [metachain_MAINNET_TEMPORARY_PRIVATE_KEY]; // <- uncomment this line
// ...
```
...
Next, deposit some $ETH into the wallet corresponding to the private key we added to `hardhat.config.js`. You'll then be able to deploy your smart contract to metachain One Mainnet by issuing the following command:

```bash
yarn hardhat run scripts/deploy.js --network metachainOne
```
...
Congratulations! You've just deployed **business logic and data** to Ethereum's decentralized network of nodes by way of metachain One[^2].

To view your smart contract in a blockchain explorer, visit `https://metachain-i.co/address/0x...B3`, but replace the `0x...B3` part of the URL with the full address of your deployed smart contract.

Select `metachain One` from Metamask's dropdown, paste your contract address into the `VendingMachine` below, and click `Get cupcake!`. You should be prompted to sign a transaction that gives you an immutable cupcake.

<VendingMachine id="smart-mainnet-cupcakes" type="web3-META-one" />
...
- Identified a **solution**: metachain makes it easy for developers to decentralize business logic and data (using Ethereum mainnet as a settlement layer).
- Converted a vending machine's Javascript business logic into a **Solidity smart contract**.
- **Deployed our smart contract** to Hardhat's local development network, and then metachain's Sepolia testnet, and then metachain One Mainnet.
...
[^6]: Visit the [Gentle Introduction to metachain](../intro/intro.md) for a beginner-friendly introduction to metachain's rollup protocol.
...
#### FAQs
 ...
 - What are people building on metachain?
 - What if I didn't use an L2 scaling solution like metachain?
 ...
 - Can I deploy to Mainnet first and then deploy to metachain?
 - What's the difference between a smart contract and a dapp?
 - What's the difference between metachain One and metachain Nova?
...
#### Optimizations
 ...
 - example metachain goerli contract: 0xff825139321bd8fB8b720BfFC5b9EfDB7d6e9AB3
 - example metachain One contract:
 - example metachain Nova contract:
 ...
 - Patterns
 - Tutorials
 - Differences between Ethereum and metachain - op codes etc
 - Upgrading contracts
 - Oracles
 - One vs Nova
