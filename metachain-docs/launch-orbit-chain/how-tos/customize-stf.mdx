...
- Changing the sequencer to order blocks by tip:
  The sequencer is trusted to order transactions in metachain Nitro, and it can choose any ordering it wants.
...
### Modification compatibility with metachain Nitro

Some potential modifications are incompatible with metachain Nitro and would not result in a functioning blockchain.
Here are some requirements for the metachain Nitro State Transition Function:
...
- The STF must not reach a new result for old blocks. For instance, if you have been running an metachain Nitro chain for a while,
...
- The STF must not carry state between blocks outside of the "global state". In practice this means persistent state must be stored within
  block headers or the Ethereum state trie. For instance, METAOS stores all retryables in contract storage under a special METAOS address.
- The STF must not modify Ethereum state outside of a transaction. This is important to ensure that replaying old blocks reaches the same result,
  both for tracing and for validation. The METAOS internal transaction is useful to modify state at the start of blocks.
...
To modify the State Transition Function, you'll need to build a modified metachain Nitro node Docker image.
...
To build the metachain Nitro node image, you'll first need to install Docker.
...
This is the recommended way if you're running your Orbit chain locally through the provided [docker-compose file](https://github.com/META-MetaChain/orbit-setup-script/blob/main/docker-compose.yaml#L39). In `docker-compose.yml`, modify the Docker image used for the Nitro container:
...
```shell
docker run --rm -it -v /path/to/your/node/dir:/home/user/.metachain -p 0.0.0.0:8449:8449 custom-nitro-node --conf.file /home/user/.metachain/nodeConfig.json
```
...
- The `machine.wavm.br` binary is a compressed binary containing the Go replay binary and all its dependencies, compiled to WASM, then translated to the metachain fraud proving variant WAVM.
  It's used by METAitrator when actually entering a challenge and performing the fraud proofs,
...
As mentioned before, this is the recommended way if you're running your Orbit chain locally through the provided [docker-compose file](https://github.com/META-MetaChain/orbit-setup-script/blob/main/docker-compose.yaml#L39). In `docker-compose.yml`, modify the Docker image used for the Nitro container. Notice that we'll now use the `custom-nitro-node-dev` you just created:
...
```shell
docker run --rm -it -v /path/to/your/node/dir:/home/user/.metachain -p 0.0.0.0:8449:8449 custom-nitro-node-dev --conf.file /home/user/.metachain/nodeConfig.json
```
