...
description: "Launch your own Orbit chain with the metachain Nitro codebase's new license. Settle to metachain's L2 chains via bridge contracts on the underlying L2 chain (Sepolia, Goerli, One or Nova). No need for permission from the metachain DAO or Offchain Labs to create your Orbit chain. Modify the Nitro codebase freely for your chain."
sidebar_position: 2
target_audience: developers who want to create their own self-managed AnyTrust or One chain
sidebar_label: 'Quickstart'
---

This quickstart is for developers who want to launch their own metachain Orbit chain using the [Orbit chain deployment portal](https://orbit.metachain-i.co/).

By the end of this quickstart, you'll have a **local devnet chain** that hosts EVM-compatible smart contracts. Your chain will process transactions locally while settling to the public **metachain Goerli or Sepolia testnets**. Familiarity with Ethereum, Ethereum's testnets, and metachain is expected.
...
## Step 1: Acquire metachain Testnet $ETH (and the native token for Orbit chains with custom gas tokens)

You'll need at least 1 testnet $ETH for regular Orbit chains or 0.6 $ETH plus 0.4 of your desired native token for Orbit chains with a custom gas token. The funds are needed to cover the cost of deploying your Orbit chain's **base contracts** to its **base chain** (metachain Goerli or Sepolia). Sepolia is our recommendation as Goerli will be deprecated in the near future.

At the time of this quickstart's writing, the easiest way to acquire $ETH is to bridge testnet $ETH from Ethereum's L1 Goerli or Sepolia network to metachain's corresponding L2 testnet:
...
2. Bridge your L1 testnet $ETH into metachain L2 using [the metachain bridge](https://bridge.metachain-i.co/).
...
<p>
  <a data-quicklook-from="metachain-rollup-protocol">metachain Rollup</a> is an Optimistic Rollup
...
By contrast, <a data-quicklook-from='metachain-anytrust-protocol'>metachain AnyTrust</a> introduces a trust assumption in exchange for lower fees. Instead of requiring all metachain nodes to have access to the data of every L2 transaction in the metachain chain's inbox, AnyTrust relies on an external <a data-quicklook-from='data-availability-committee-dac'>Data Availability Committee</a> to store data and provide it on demand, reducing the costs of batching and posting all L2 transaction data to L1. Visit our [FAQ docs](../learn-more/faq) to learn more.
...
Visit the [Orbit chain deployment portal](https://orbit.metachain-i.co/). You'll be prompted to connect your wallet. You may be prompted to add the **metachain Goerli** or **metachain Sepolia** network to your wallet and/or to switch your wallet to this network; approve this.
...
| **Challenge period (blocks)** | The amount of time that your Orbit chain's nodes have to dispute the current state of the chain before it's confirmed (and ultimately finalized) on the underlying L2 chain (metachain Goerli or Sepolia). Note that this refers to the number of blocks on the underlying L1 chain (Ethereum's Goerli or Sepolia chain).                                                                                                                                                         |
| **Stake token**               | The token that your chain's validators must stake in order to participate in your chain. This is hardcoded to $ETH for now, but future versions of Orbit chains will let you specify an METAitrary ERC-20 token contract here.                                                                                                                                                                                                                                                    |
...
Deploy your chain's base contracts to metachain Goerli or Sepolia

<!-- todo: label-casing alignment - could sentence-case in UI -->

Click the `Deploy` button located below the config form. Your wallet should prompt you to submit a transaction to the metachain testnet. You'll have to pay a little gas; your wallet may denominate this in $ETH; as long as you see your chosen metachain testnet in the transaction details, this gas fee will be paid in testnet $ETH.

Before proceeding, let's briefly review what just happened:

- You submitted a deployment transaction to an Orbit "factory" smart contract on the metachain testnet, the public L2 chain that your local Orbit chain will settle transactions to.
- This Orbit smart contract then initialized your Orbit chain's base contracts with the values that you specified in the previous step, and deployed these base contracts to the metachain testnet.
...
Click the `Deploy` button located below the Deployment Summary. Again, your wallet should prompt you to submit a transaction to the metachain testnet. Gas fees will be handled similarly to the previous transaction.
...
1.  Clone the [orbit-setup-script](https://github.com/META-MetaChain/orbit-setup-script) repository: `git clone https://github.com/META-MetaChain/orbit-setup-script.git`
...
Using metachain Goerli:

```shell
PRIVATE_KEY="0xYourPrivateKey" L2_RPC_URL="https://goerli-rollup.metachain-i.co/rpc" L3_RPC_URL="http://localhost:8449" yarn run setup
```

Using metachain Sepolia:

```shell
PRIVATE_KEY="0xYourPrivateKey" L2_RPC_URL="https://sepolia-rollup.metachain-i.co/rpc" L3_RPC_URL="http://localhost:8449" yarn run setup
```
...
```shell
PRIVATE_KEY="0xYourPrivateKey" L2_RPC_URL="https://goerli-rollup.metachain-i.co/rpc" L3_RPC_URL="http://localhost:8449" AMOUNT="<AMOUNT>" yarn run deposit
```

Using metachain Sepolia:

```shell
PRIVATE_KEY="0xYourPrivateKey" L2_RPC_URL="https://sepolia-rollup.metachain-i.co/rpc" L3_RPC_URL="http://localhost:8449"
