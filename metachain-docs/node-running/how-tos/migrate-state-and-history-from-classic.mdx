...
author: jose-franco
sme: jose-franco
target_audience: 'Node runners interested in creating the full state of the metachain One chain on their own (i.e., without an initial database)'
content-type: how-to
sidebar_position: 11
---

When running a Nitro node for the first time on a chain that produced [classic blocks](/for-devs/concepts/public-chains#classic-deprecated) in the past (like metachain One), you need to initialize its database to, at least, the state of the chain after executing the last classic block. The common, and recommended, way of doing that is to provide a database snapshot using the `--init.url` option (as mentioned in [How to run a full node (Nitro)](/node-running/how-tos/running-a-full-node.mdx)). In this how-to we show you an alternative way for doing that, migrating the state and history of the chain from a fully synced classic node.
...
Keep in mind that this process only applies to metachain One. Other metachain chains didn't produce classic blocks in the past, they started as Nitro chains.
...
Launch your classic node with the option `--node.rpc.nitroexport.enable=true`. All exported data will be written to directory "nitroexport" under the classic instance directory (e.g. `${HOME}/.metachain/mainnet/nitroexport`). Make sure the classic node has read the entire rollup state.
...
- Call the RPC method `META_exportHistory` with parameter `"latest"` to initiate history export. It will return immediately.
- Calling `META_exportHistoryStatus` will return the latest block exported, or an error if the export failed.
...
- Call the RPC method `META_exportState` with parameter `"latest"` to initiate state export. Unless disconnected, this will only return after the state export is done.
...
- Call the RPC method `META_exportOutbox` with parameter `"0xffffffffffffffff"` to initiate outbox export. It will return immediately.
- Calling `META_exportOutboxStatus` will return the latest outbox batch exported, or an error if the export failed.
...
- Place the `l2chaindata` and `classic-msg` (if exported) directories in Nitro's instance directory (e.g. `${HOME}/.metachain/META1-nitro/`).
