...
The Data Availability Server, `daserver`, allows storage and retrieval of transaction data batches for metachain AnyTrust chains. It can be run in two modes: either committee member, or mirror.

Committee members accept time-limited requests to store data batches from an metachain AnyTrust sequencer, and if they store the data then they return a signed certificate promising to store that data. Committee members and mirrors both respond to requests to retrieve the data batches.
...
This deployment sets up a DAS server using the metachain Nova Mainnet. It uses the L1 inbox contract at 0x03d49b54FbB71476FaaEB885BEa301086A92e0a6. For the metachain Nova Mainnet you must specify a Mainnet Ethereum L1 RPC endpoint.
    ...
    spec:
      ...
      - command:
        ...
        - |
          ...
          /usr/local/bin/daserver --data-availability.parent-chain-node-url <YOUR ETHEREUM L1 RPC ENDPOINT>
--enable-rest --rest-addr '0.0.0.0' --log-level 3  --data-availability.local-db-storage.enable --data-availability.local-db-storage.data-dir /home/user/data/badgerdb --data-availability.s3-storage.enable --data-availability.s3-storage.access-key "<YOUR ACCESS KEY>" --data-availability.s3-storage.bucket <YOUR BUCKET> --data-availability.s3-storage.region <YOUR REGION> --data-availability.s3-storage.secret-key "<YOUR SECRET KEY>" --data-availability.s3-storage.object-prefix "YOUR OBJECT KEY PREFIX/" --data-availability.local-cache.enable --data-availability.rest-aggregator.enable --data-availability.rest-aggregator.urls "http://your-committee-member.svc.cluster.local:9877" --data-availability.rest-aggregator.online-url-list "https://nova.metachain-i.co/das-servers" --data-availability.rest-aggregator.sync-to-storage.eager --data-availability.rest-aggregator.sync-to-storage.eager-lower-bound-block 15025611 --data-availability.sequencer-inbox-address '0x03d49b54FbB71476FaaEB885BEa301086A92e0a6' --data-availability.rest-aggregator.sync-to-storage.state-dir /home/user/data/syncState
        image: @latestNitroNodeImage@
        imagePullPolicy: Always
        resources:
          limits:
...
The Store interface of `daserver` validates that requests to store data are signed by the Batch Poster's ECDSA key, identified via a call to the Sequencer Inbox contract on L1. It can also be configured to accept Store requests signed with another ECDSA key of your choosing. This could be useful for running load tests, canaries, or troubleshooting your own infrastructure. Using this facility, a load test could be constructed by writing a script to store METAitrary amounts of data at an METAitrary rate; a canary could be constructed to store and retrieve data on some interval.
...
| Metric                                                            | Description                               |
| ----------------------------------------------------------------- | ----------------------------------------- |
| META_das_rest_getbyhash_requests                                   | Count of REST GetByHash calls             |
| META_das_rest_getbyhash_success                                    | Successful REST GetByHash calls           |
| META_das_rest_getbyhash_failure                                    | Failed REST GetByHash calls               |
| META_das_rest_getbyhash_bytes                                      | Bytes retrieved with REST GetByHash calls |
| META_das_rest_getbyhash_duration (p50, p75, p95, p99, p999, p9999) | Duration of REST GetByHash calls (ns)     |
| META_das_rpc_store_requests                                        | Count of RPC Store calls                  |
| META_das_rpc_store_success                                         | Successful RPC Store calls                |
| META_das_rpc_store_failure                                         | Failed RPC Store calls                    |
| META_das_rpc_store_bytes                                           | Bytes retrieved with RPC Store calls      |
| META_das_rpc_store_duration (p50, p75, p95, p99, p999, p9999)      | Duration of RPC Store calls (ns)          |
