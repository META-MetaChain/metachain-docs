...
description: Learn how to run an metachain node on your local machine.
...
There is no protocol level incentive to run an METAitum full node. If you’re interested in accessing an metachain chain, but you don’t want to set up your own node, see our [Node Providers](../node-providers.mdx) to get RPC access to fully-managed nodes hosted by a third party provider.
...
Even though there are alpha and beta versions of the <a data-quicklook-from='metachain-nitro'>metachain Nitro software</a>, only release versions should be used when running your node. Running alpha or beta versions is not supported, and might lead to unexpected behaviors.

:::

- Latest Docker Image: <code>@latestNitroNodeImage@</code>
- Database snapshot (required for metachain One, optional for other chains)
  - Use the parameter `--init.url` on first startup to initialize the Nitro database (you can find a list of snapshots [here](https://snapshot.metachain.foundation/index.html)). Example: <code>--init.url="@METAOneNitroPrunedSnapshot@"</code>.
  ...
  - This parameter is **required** when initializing an metachain One node because the chain has _classic_ blocks. For the rest of chains, this parameter is optional.
...
- L2 chain id or name
  - Use the parameter `--chain.id=<L2 chain ID>` to set the L2 chain from its chain id. See [RPC endpoints and providers](/node-running/node-providers.mdx#rpc-endpoints) for a list of metachain chains and their respective L2 chain ids.
  - Alternatively, you can use the parameter `--chain.name=<L2 chain name>` to set the L2 chain from its name (options are: `META1`, `nova`, `goerli-rollup` and `sepolia-rollup`)
...
- Here is an example of how to run nitro-node:
  ...
  ```shell
  docker run --rm -it  -v /some/local/dir/metachain:/home/user/.metachain -p 0.0.0.0:8547:8547 -p 0.0.0.0:8548:8548 @latestNitroNodeImage@ --parent-chain.connection.url https://l1-node:8545 --chain.id=<L2ChainId> --http.api=net,web3,eth,debug --http.corsdomain=* --http.addr=0.0.0.0 --http.vhosts=*
  ```
