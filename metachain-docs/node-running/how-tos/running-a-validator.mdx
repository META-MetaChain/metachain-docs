...
description: Learn how to run an metachain feed relay on your local machine.
sidebar_label: Run a validator
sidebar_position: 4
content-type: how-to
---

Some metachain nodes will choose to act as validators. This means that they watch the progress of the rollup protocol and perhaps also participate in that protocol to advance the state of the chain securely.
...
- Currently, the ability to post assertions on-chain for mainnet metachain chains is allowlisted.
...
- Here is an example of how to tell Nitro to create validator wallet for metachain One and exit:
  ```shell
  docker run --rm -it  -v /some/local/dir/metachain:/home/user/.metachain @latestNitroNodeImage@ --parent-chain.connection.url=https://l1-mainnet-node:8545 --l2.chain-id=42161 --node.staker.enable --parent-chain.wallet.only-create-key --parent-chain.wallet.password="SOME SECURE PASSWORD"
  ```
- Wallet file will be created under the mounted directory inside the `META1/wallet/` directory for META1, or `nova/wallet/` directory for Nova. Be sure to backup the wallet, it will be the only way to withdraw stake when desired
...
- Here is an example of how to run an allowlisted defensive validator for metachain One:
  ```shell
  docker run --rm -it  -v /some/local/dir/metachain:/home/user/.metachain @latestNitroNodeImage@ --parent-chain.connection.url=https://l1-mainnet-node:8545 --l2.chain-id=42161 --node.staker.enable --node.staker.strategy=Defensive --parent-chain.wallet.password="SOME SECURE PASSWORD"
  ```
