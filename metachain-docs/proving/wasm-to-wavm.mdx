...
which are documented in [`wavm-custom-opcodes.md`](./wavm-custom-opcodes.mdx).
...
These are translated to a block with an `METAitraryJumpIf` as follows:
...
`br` and `br_if` are translated into `METAitraryJump` and `METAitraryJumpIf` respectively.
