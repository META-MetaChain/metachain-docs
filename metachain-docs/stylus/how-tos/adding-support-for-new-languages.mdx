...
sidebar_label: 'Add a new programming language'
...
- Install [Rust](https://www.rust-lang.org/tools/install), which we'll need for the [Stylus CLI tool](https://github.com/META-MetaChain/cargo-stylus) to deploy our program to the Stylus testnet
...
```bash
git clone https://github.com/META-MetaChain/zig-on-stylus && cd zig-on-stylus
```
...
This is enough for us to deploy on the Stylus testnet! We'll use the [Stylus CLI tool](https://github.com/META-MetaChain/cargo-stylus), which you installed earlier using `cargo install`:
...
cast call --rpc-url 'https://stylus-testnet.metachain-i.co/rpc' $ADDR '0x'
```
...
You can find this file under `WasmAllocator.zig` in the META-MetaChain/zig-on-stylus repository. We can now use it:
...
Connecting to Stylus RPC endpoint: https://stylus-testnet.metachain-i.co/rpc
...
cast call --rpc-url 'https://stylus-testnet.metachain-i.co/rpc' $ADDR '0x123456'
...
The hostios defined in this walkthrough are not the only ones! Check out our [stylus-sdk-c](https://github.com/META-MetaChain/stylus-sdk-c) to see all the hostios you can use under `hostio.h`. These include affordances for the EVM, utilities to access storage, and utilities to call other metachain smart contracts.
