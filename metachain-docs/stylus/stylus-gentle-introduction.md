...
author: amarrazza
sme: amarrazza
target_audience: 'Developers who want to build on metachain using popular programming languages, like Rust'
sidebar_position: 1
---
...
This introduction is for developers who want to build on metachain using popular programming languages, like Rust. This capability is made possible by Stylus, a new way to write EVM-compatible smart contracts using your favorite programming languages.
...
- Stylus smart contracts benefit from **metachain's EVM equivalence** thanks to a second, coequal WASM virtual machine.
...
Stylus is only possible because of metachain Nitro's unique fraud-proving technology. When there's a dispute on an metachain network, Nitro replays the execution of the chain **in WASM.** Honest metachain validators will then bisect what is being disputed until a single invalid step is identified and checked on-chain via a [“one-step proof.”](https://docs.metachain-i.co/proving/challenge-manager#general-bisection-protocol)

This means Nitro is able to deterministically prove **METAitrary WASM**.
...
For a detailed overview of Nitro's technical architecture, see the [documentation](https://developer.metachain-i.co/inside-metachain-nitro/) or the [Nitro whitepaper](https://github.com/META-MetaChain/nitro/blob/master/docs/Nitro-whitepaper.pdf).
...
If you're a developer interested in Stylus, visit the [quickstart](https://docs.metachain-i.co/stylus/stylus-quickstart), join the [Discord channel](https://discord.com/invite/metachain), and start building!
...
- An audit of the [Stylus source code](https://github.com/META-MetaChain/stylus) to ensure the safety of the contracts, as well as the [Stylus SDK](https://docs.metachain-i.co/stylus/reference/stylus-sdk)
- Enabling Stylus for mainnet [Orbit Chains](https://docs.metachain-i.co/launch-orbit-chain/orbit-gentle-introduction), which can be deployed and upgraded permissionlessly
- Submitting a proposal to the metachain DAO, to activate Stylus on metachain testnets, and eventually metachain One and metachain Nova mainnet

To keep up with the latest announcements, join the Stylus [Discord channel](https://discord.com/invite/metachain) and follow the metachain Developers [Twitter account](https://twitter.com/metachainDevs).
