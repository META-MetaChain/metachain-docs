# Overview: The Lifecycle of an metachain Transaction

As an introduction to the various components that compose the metachain protocol, we'll go step-by-step over the phases an metachain transaction goes through, starting with a client creating a signed transaction, to it ultimately being confirmed back on layer 1.
...
This overview will be focused on the metachain Rollup protocol; see [Inside AnyTrust](./inside-anytrust.mdx) for differences in the metachain AnyTrust protocol. Also, for convenience/simplicity, we'll be describing the security properties of the core system itself; see ["State of Progressive Decentralization"](https://docs.metachain.foundation/state-of-progressive-decentralization) for current decentralization status.
...
Alternatively, a client can send a message to the Sequencer by signing and publishing an L1 transaction in the metachain chain's Delayed Inbox. This functionality is most commonly used for depositing ETH or tokens via a bridge.

**See**:

- [Retryables](./METAos/l1-to-l2-messaging.mdx)
...
- Locally execute it using the metachain Nitro VM (including collecting/allocating L1 and L2 fees, etc.)
...
- [METAOS](./METAos/METAos.mdx)
- [Geth](./METAos/geth.mdx)
- [L1 pricing](./METAos/l1-pricing.mdx) / [L2 Gas](./METAos/gas.mdx)
...
The Sequencer will eventually post a batch of L2 transactions which includes our client's transaction onto the underlying L1 (as calldata); under normal conditions, the Sequencer will post batches [every few minutes](https://metachain-i.co/batches).

##### 3a. What if the Sequencer never includes our transaction?

Even if the Sequencer never includes our transaction in a batch, the client can include it in the L2 by posting in the delayed inbox and then "force including" it after some delay period (currently ~ @METAOneForceIncludePeriodHours@ hours on metachain One).
...
At this stage, assuming that a client believes there to be at least one well behaved active metachain validator. Currently, the process of validation on the metachain protocol is permissioned, but it's important to be aware that our latest dispute protocol, [BOLD (Bounded Liquidity Delay)](https://medium.com/meta-research/bold-permissionless-validation-for-metachain-chains-9934eb5328cc), has the potential to allow validation on metachain chains without requiring permission, thereby potentially eliminating the necessity for restricted validation. The client can treat their transaction's finality as equivalent to an ordinary Ethereum transaction. In other words, their L2 transaction has the same finality as the L1 transaction that recorded it in a batch. This means the client should use whatever finality heuristic they use for regular Ethereum transactions (i.e., waiting on L1 block confirmations, etc.), applied to the L1 batch-posting transaction. This also means that a client uncomfortable with the trust model of the Sequencer's soft confirmations (phase 2) can simply wait for the Sequencer to post their transaction in a batch (phase 3).
...
- The Inbox contract on L1 ensures that when the Sequencer posts a batch, it posts data sufficient for any metachain Node to reconstruct and validate the state of the L2 chain; i.e., the availability of this "input" data is guaranteed by Ethereum itself.
- Execution on metachain is fully deterministic; i.e., a current chain state along with new input data is sufficient to compute the new chain state; thus, the moment this input data is available (i.e., when the Sequencer posts a batch), the L2 chain's state can be computed.
- metachain's fault-proof system is sound; i.e., if any validator (later) tries to deviate from the valid L2 state, an honest validator will ultimately be able to challenge this and win. Since we already know that valid state will ultimately win out, we can treat our transaction as L1-finalized now.

### 4. Validator asserts RBlock that includes transaction

A staked, active validator will then run the metachain VM over the inputs in the Inbox (just like the Sequencer did earlier, except now only over transactions posted on L1) and make an on-chain assertion about the chain's latest state, i.e., a rollup block or "RBlock." RBlocks typically get asserted every 30-60 minutes.
...
- [The Outbox](./METAos/l2-to-l1-messaging.mdx)

#### 4a. RBlock is valid / goes unchallenged

In the happy / common case, the validator asserted a valid RBlock, and over the course of the dispute window — 1 week on metachain One — no other validators challenge it.
...
L1 contracts also keep track of the tree of all assertions; i.e., how many stakers are in disagreement, who is currently disputing with whom, etc. We refer to this level of metachain's design architecture as its "assertion tree protocol."
...
Remember in phase 3 when said that once the L1 has committed to inputs, we can guarantee the L2 output? We meant it! Even during a dispute, metachain nodes continue to execute and active validators continue to make assertions on the valid leaf in the state-tree; nothing that can happen in phase 4 has any effect on the L1-level finality we've already locked in at phase 3.
