...
import {
  ...
  LinkableTerms,
  LinkValidity,
} from '@META-MetaChain/notion-docs-generator'
import fs from 'fs'
...
const getContentFromCMS = async (): Promise<CMSContents> => {
  const devDocsV2Project = await lookupProject(
    notion,
    'metachain developer docs portal v2.0'
  )
      ...
      and: [
        {
          ...
          multi_select: {
            contains: 'troubleshooting-using-metachain',
          },
        },
        {
...
async function generateFiles() {
  ...
  fs.writeFileSync(
    '../metachain-docs/partials/_glossary-partial.md',
    definitionsHTML
  )
